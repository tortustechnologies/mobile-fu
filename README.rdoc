...
To automatically set session[:mobile_view] using params[:format], pass the option
:setup_session => true into has_mobile_fu. In this way, accessing a url with the
query string "format=mobile" or "format=html" will permanently switch the site's
